...
export default function Invoice({ appendData }) {
    ...
    return(
        <div className="phone:w-full phone:h-full md:h-full phone:absolute phone:top-[60px] h-full md:w-invoice overflow-hidden md:fixed md:left-[60px] z-40 md:top-0 md:rounded-r-[1.25rem] bg-[#fff] px-14 pt-10 shadow-logobg shadow-2xl dark:bg-[#141625] ">
            <h1 className="text-3xl mb-9 dark:text-[#fff]">Yeni Fatura</h1>
            <form method="post" autoComplete="off" onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
                ...
                <input type="date" id="billDate" name="billDate" required className="h-8 w-[9.5rem] rounded-sm px-1 border-[#DFE3FA] border border-solid outline-none dark:bg-[#1E2139] dark:text-[#fff] dark:border-[#252945]"/>
                <label className="dark:text-white">Fatura ödeme aracı :</label>
                <input type="text" id="paymentMethod" name="paymentMethod" placeholder="İş bankası kartım" required className="h-10 w-[9.5rem] rounded-sm px-1 border-[#DFE3FA] border border-solid outline-none dark:bg-[#1E2139] dark:text-[#fff] dark:border-[#252945]" />
                <label className="dark:text-white">Fatura bedeli giriniz :</label>
                <input type="number" name="amount" id="amount" required inputMode="numeric" placeholder="350" className="h-10 w-[9.5rem] px-1 rounded-sm border-[#DFE3FA] border border-solid outline-none dark:bg-[#1E2139] dark:text-[#fff] dark:border-[#252945]" />   
